...
void loadmodel(void)
{
	if (pmodel == NULL)
	{
		pmodel = glmReadOBJ("Models/porsche.obj");
...
void renderScene2(int currentWindow)
{
	...
	glTranslatef(x, y - 1.2, z); //Coloca o carro no solo
...
void renderScenesw1(void)
{
	glutSetWindow(subWindow1);
	glLoadIdentity();
	gluLookAt(x + 7 * -sin(angle), y + 1, z + 7 * cos(angle),
		x + lx, y + ly, z + lz,
		0.0f, 1.0f, 0.0f);
...
int main(int argc, char **argv)
{
	...
	mainWindow = glutCreateWindow("Programa-08");
